...
class Admin(commands.Cog):
    ...
    async def log(self, ctx, amount : int = None, moderator : discord.Member = None):
        ...
        def editPage(moderator, embedPage):              # 임베드 정의 함수 (사용자 및 페이지 정의)
            if moderator == None:
                return discord.Embed(title="감사로그", description="\n\n" + logList[embedPage], color=0x000000)
            else:
                return discord.Embed(title=moderator.name + "님의 감사로그", description="\n\n" + logList[embedPage], color=0x000000)

        async for entry in ctx.guild.audit_logs(limit=amount, user=moderator):
            if entry.target == None:
                log += "`" + str(logIndex) + ".` " + "{0.user.mention}님이 {0.action}을 했습니다.\n".format(entry)
            else:
                log += "`" + str(logIndex) + ".` " + "{0.user.mention}님이 {0.target}에게 {0.action}을 했습니다.\n".format(entry)
