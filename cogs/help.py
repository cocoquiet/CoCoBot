...
class Help(commands.Cog):
    ...
    async def help(self, ctx, *, args : str = None):
        if args == None:
            ...
            embed.add_field(name="`/help`", value="> 명령어들의 종류를 크게 구분해서 보여줄게요.", inline=False)
            embed.add_field(name="`/help general`", value="> 기본적인 명령어들을 보여줄게요.", inline=False)
            embed.add_field(name="`/help game`", value="> 저를 통해 할 수 있는 게임의 명령어들을 보여줄게요.", inline=False)
            embed.add_field(name="`/help music`", value="> 음악과 관련된 명령어들을 보여줄게요.", inline=False)
            embed.add_field(name="`/help admin`", value="> 관리자만 사용 가능한 명령어들을 보여줄게요.", inline=False)
            ...
            embed = discord.Embed(title="게임 명령어", description="저를 통해 할 수 있는 게임의 명령어들을 알려줄게요!", color=0xffffff)
            embed.add_field(name="`/가위바위보`", value="> 가위바위보를 해줄게요.", inline=False)
            embed.add_field(name="`/오목 <다른 플레이어> <모드 번호>`", value="> 다른 사람과 오목을 하게 해줄게요.\n모드 번호는 `/오목 모드`로 알려줄게요.", inline=False)
            embed.set_footer(text="코코#1174 | V1.5")
            ...
            embed.add_field(name="`/play`", value="> 원하는 노래를 들려줄게요.", inline=False)
            embed.add_field(name="`/stop`", value="> 듣고 있는 음악을 멈춰줄게요.", inline=False)
            embed.add_field(name="`/volume`", value="> 음악의 볼륨을 조절해줄게요.", inline=False)
            embed.add_field(name="`/음악목록`", value="> 음악 재생목록 링크를 모아서 보여줄게요.", inline=False)
            embed.set_footer(text="코코#1174 | V1.5")
            ...
            embed.add_field(name="`/kick <추방할 유저>`", value="> 문제가 있는 사람들을 추방시켜줄게요.", inline=False)
            embed.add_field(name="`/ban <차단할 유저>`", value="> 마음에 들지 않은 사람들을 차단시켜 줄게요.", inline=False)
            embed.add_field(name="`/청소 <삭제할 개수>`", value="> 많은 메세지를 한번에 지워줄게요.\n> 개수를 입력하지 않으면 자동으로 10개만 지워줄게요.", inline=False)
            embed.add_field(name="`/고코위`", value="> 모든 어드민들을 불러줄게요.", inline=False)
            embed.set_footer(text="코코#1174 | V1.5")
